...
fun getExtraBundle(queue: LongArray, title: String, seekTo: Int?): Bundle? {
    val bundle = Bundle()
    bundle.putLongArray(MediaConst.SONGS_LIST, queue)
    bundle.putString(MediaConst.QUEUE_TITLE, title)
    if (seekTo != null)
        bundle.putInt(MediaConst.SEEK_TO_POS, seekTo)
    else bundle.putInt(MediaConst.SEEK_TO_POS, 0)
    return bundle
}
